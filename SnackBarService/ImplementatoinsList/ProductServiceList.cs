...
{
    public class ProductServiceList : InterfaceProductService
    {
        ...
        public List<ModelProductView> getList()
        {
            ...
            {
                List<ModelProdComponentView> productElements = new List<ModelProdComponentView>();
                for (int j = 0; j < source.ProductElements.Count; ++j)
                {
                    if (source.ProductElements[j].ProductID == source.Products[i].ID)
                    {
                        ...
                        productElements.Add(new ModelProdComponentView
                        {
        ...
        public ModelProductView getElement(int id)
        {
            for (int i = 0; i < source.Products.Count; ++i)
            {
                List<ModelProdComponentView> productComponents = new List<ModelProdComponentView>();
                for (int j = 0; j < source.ProductElements.Count; ++j)
                {
                    if (source.ProductElements[j].ProductID == source.Products[i].ID)
                    {
                        ...
                        productComponents.Add(new ModelProdComponentView
                        {
                            ID = source.ProductElements[j].ID,
                            ProductID = source.ProductElements[j].ProductID,
                            ElementID = source.ProductElements[j].ElementID,
        ...
        public void addElement(BoundProductModel model)
        {
            int maxID = 0;
            for (int i = 0; i < source.Products.Count; ++i)
            {
            ...
            for (int i = 0; i < model.ProductElements.Count; ++i)
            {
                source.ProductElements.Add(new ProductElement
                {
        ...
        public void updateElement(BoundProductModel model)
        {
            int index = -1;
            for (int i = 0; i < source.Products.Count; ++i)
            {
